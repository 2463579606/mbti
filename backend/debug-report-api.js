...
async function debugReportAPI() {
  try {
    // Use the most recent report (ID: 2)
    const reportId = 2;
    const sessionToken = 'sess_7cedf30cb399b77f'; // From the previous test
    ...
    if (response.data) {
      ...
      if (response.data.mbtiType) {
        ...
        console.log('Code:', response.data.mbtiType.code);
        console.log('Name:', response.data.mbtiType.name);
        console.log('Group:', response.data.mbtiType.group);
        console.log('Headline:', response.data.mbtiType.headline);
        console.log('Tagline:', response.data.mbtiType.tagline);
        console.log('Emoji:', response.data.mbtiType.emoji);
      ...
      if (response.data.dimensions) {
        console.log('Dimensions:', JSON.stringify(response.data.dimensions, null, 2));
      }
